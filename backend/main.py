...
async def predict(data: ClaimData):
    ...
    try:
        ...
        input_data = {
            ...
            "diagnosis_code": 238 if unapproved_ratio > 0.7 else 50, # 238 is a known high-risk case code
            "claim_amount": data.claim_amount,
            "approved_amount": data.approved_amount,
            "hospital_stay_days": 28 if unapproved_ratio > 0.6 else 4, 
            "previous_claims_count": 8 if unapproved_ratio > 0.7 else 1, 
            "policy_tenure_years": 10 if unapproved_ratio > 0.5 else 2,
            "claim_submission_delay_days": 35 if unapproved_ratio > 0.5 else 5,
            "high_risk_procedure_flag": 1 if unapproved_ratio > 0.8 else 0,
            "document_mismatch_flag": 1 if unapproved_ratio > 0.7 else 0,
            "anomaly_score": min(0.99, unapproved_ratio + 0.1),
